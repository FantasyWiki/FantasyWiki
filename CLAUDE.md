...
Runtime is a Cloudflare Worker using Hono (`backend/src/index.ts`). A deployment persists to **either** Cloudflare D1 (the `db` binding, the default) **or** MongoDB (`PERSISTENCE=mongo` plus `MONGO_URL`); `composition.ts` is the only module that picks, and it is synchronous because the request middleware, the settlement Workflow and the test seam all call it without awaiting. The Mongo driver runs inside workerd — `nodejs_compat` is on in `wrangler.jsonc` for it. See `docs/architecture/persistence-targets.md`. When instantiating Hono, pass `CloudflareBindings` as the generic: `new Hono<{ Bindings: CloudflareBindings }>()`.
