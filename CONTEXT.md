...
**Invitation Code**:
The five-character code that opens a **private** league: digits and capitals minus the look-alikes (`0 O 1 I L U`). It is a credential, so it never travels on a league DTO — only the endpoint that checks the league's **Invite Policy** serves it (ADR 0008). For a public league it is a share shortcut, not a requirement.
...
- An **Invitation Code** belongs to exactly one **League** and is unique across all of them
