...
export class PerformanceRepositoryD1 implements PerformanceRepository {
  ...
  async getLeagueCumulatives(
    ...
    try {
      ...
      // the derivation once. The view is team-anchored, so a team with no
      // contracts is already present with the full starting budget — this
      // joins it plainly, no COALESCE fallback needed.
