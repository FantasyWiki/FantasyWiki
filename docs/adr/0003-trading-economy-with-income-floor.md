> **Pricing formula superseded:** the `Normalized 30-day-average Views / 1000 × weeks` formula below is replaced by the convex, days-based formula in [ADR 0005](0005-convex-contract-pricing.md). Everything else on this page (stipend, transaction fee, renewal premium, minimum hold, wealth ceiling) is unaffected and still canonical.
...
- **Pricing (superseded by [ADR 0005](0005-convex-contract-pricing.md)):** ~~`Normalized 30-day-average Views / 1000 × weeks`~~ — priced on the *smoothed* average, so daily spikes are cheap but fleeting (the anti-jackpot guard) and sustained popularity is what's expensive. This principle still holds; only the exact formula (now convex-in-views, days-based) changed.
