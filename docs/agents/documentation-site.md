...
- **No em dashes.** Use a comma, a colon, a semicolon, brackets or a full stop,
  whichever the sentence actually needs. An em dash is not punctuation this
  documentation uses, and a page that reaches for one reads as machine-written
  rather than considered. Where the dash was carrying real work, the fix is
  usually two sentences rather than one substitute mark.
